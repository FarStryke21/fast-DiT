...
## Retired course-era sweep scripts

`ablations.py`, `evaluation.sh`, and `time_ablation.sh` are this project's own code (not inherited from
upstream) — the original course-era sweep entry points. They are superseded on this branch by
`run_experiments.py`, the manifest-driven, resumable runner at the repository root, and are kept here only
for provenance.
